...
pub async fn create_decision(id: &str, decision: &Decision) -> Result<()> {
    db()
        .await
        .query("UPSERT type::record('decision', $id) CONTENT $data")
...
pub async fn create_decision_session(id: &str, session: &DecisionSession) -> Result<()> {
    db()
        .await
        .query("UPSERT type::record('decision_session', $id) CONTENT $data")
        .bind(("id", id.to_string()))
...
pub async fn create_trigger(id: &str, trigger: &Trigger) -> Result<()> {
    db()
        .await
        .query("UPSERT type::record('trigger', $id) CONTENT $data")
        .bind(("id", id.to_string()))
...
pub async fn relate_considered(
    ...
    db()
        .await
        .query(
            "RELATE (type::record('decision_session', $session))->considered->(type::record($table, $target)) \
...
pub async fn relate_prompted(trigger_id: &str, session_id: &str) -> Result<()> {
    db()
        .await
        .query(
            "RELATE (type::record('trigger', $trigger))->prompted->(type::record('decision_session', $session))",
        )
...
pub async fn get_decision_session(id: &str) -> Result<Option<DecisionSession>> {
    let mut resp = db()
        .await
        .query("SELECT * FROM type::record('decision_session', $id)")
        .bind(("id", id.to_string()))
        .await?;
...
pub async fn get_decision(id: &str) -> Result<Option<Decision>> {
    let mut resp = db()
        .await
        .query("SELECT * FROM type::record('decision', $id)")
        .bind(("id", id.to_string()))
        .await?;
    ...
    let mut resp = db()
        ...
        .query(
            "SELECT selected, rejection_reason, <string>out AS target FROM considered \
             WHERE in = type::record('decision_session', $id)",
        )
...
pub async fn resolve_decision_session(
    ...
    db()
        .await
        .query(
            "UPDATE type::record('decision_session', $id) SET \
...
// 直結しているため、本番データを汚さないよう isolated な RocksDb インスタンスに
...
mod tests {
    ...
    async fn list_considered_returns_related_targets_as_strings() {
        ...
        db.query("UPSERT type::record('decision_session', 'sess1') CONTENT { question: 'q', status: 'exploring', selection_mode: 'exclusive', action_taken: 'none' }")
            .await
            .expect("session upsert失敗")
            .check()
            .expect("session upsertにクエリエラー");
        db.query("UPSERT type::record('decision', 'dec1') CONTENT { hypothesis: 'h', method: 'pearson', confirmed: true, based_on_doc_ids: [] }")
            .await
            .expect("decision upsert失敗")
            .check()
            .expect("decision upsertにクエリエラー");
        db.query(
            "RELATE (type::record('decision_session', 'sess1'))->considered->(type::record('decision', 'dec1')) \
        ...
        let mut resp = db
            .query("SELECT selected, rejection_reason, <string>out AS target FROM considered WHERE in = type::record('decision_session', 'sess1')")
            .await
    ...
    async fn list_decision_sessions_includes_record_id() {
        ...
        db.query("UPSERT type::record('decision_session', 'sessA') CONTENT { question: 'qA', status: 'exploring', selection_mode: 'exclusive', action_taken: 'none' }")
            .await
            .expect("upsert失敗")
            .check()
            .expect("upsertにクエリエラー");
        db.query("UPSERT type::record('decision_session', 'sessB') CONTENT { question: 'qB', status: 'resolved', selection_mode: 'composite', action_taken: 'bought' }")
            .await
            .expect("upsert失敗")
            .check()
            .expect("upsertにクエリエラー");
